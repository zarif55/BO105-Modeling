...
for k = 1:3
    for m = 1:6
        for p = 1:6
            M_yF_HtFF = 0;
            M_xF_HtFF = 0;
            beta_1c_HtFF =((-x_cg/h)+(M_yF_HtFF/(W*h))+(CH_TPP_HtFF/CT))/(1+(((vb^2)-1)/(lock*(2*h*CT/R/sigma/a)))); %%
            beta_1s_HtFF =((y_cg/h)-(M_xF_HtFF/(W*h))+(CY_TPP_HtFF/CT))/(1+(((vb^2)-1)/(lock*(2*h*CT/R/sigma/a)))); %%
            alpha_shaft_HtFF =((((x_cg/h)-(M_yF_HtFF/(W*h))+(((((vb^2)-1)/(lock*(2*h*CT/R/sigma/a))))*CH_TPP_HtFF/CT))) /...
                (1+(((vb^2)-1)/(lock*(2*h*CT/R/sigma/a))))) +((1/2)*(f_HtFF(k)/A)*(mu_HtFF(m)^2)/CT);
            phi_shaft_HtFF =((((y_cg/h)-(M_xF_HtFF/(W*h))-(CY_TPP_HtFF*((((vb^2)-1)/(lock*(2*h*CT/R/sigma/a))))/CT)))/...
                (1+(((vb^2)-1)/(lock*(2*h*CT/R/sigma/a))))) - (CP_HtFF*R/CT/L_tr);
            theta_knot_HtFF = (((6*CT/sigma/a)*(1+(3/2*mu_HtFF(m)^2)))...
                -((0.75*theta_tw)*(1-(1.5*mu_HtFF(m)^2)+(1.5*mu_HtFF(m)^4))) + ...
                (1.5*lambda_TPP_HtFF(m))*(1-(0.5*mu_HtFF(m)^2))) + ...
                ((4/lock)*mu_HtFF(m)*((vb^2)-1)*beta_1s_HtFF) / ...
                (1 - (mu_HtFF(m)^2) +(9/4*mu_HtFF(m)^4)) ;
            theta_1s_HtFF = -beta_1c_HtFF +((1/(1+(1.5*mu_HtFF(m)^2)))*((-8/3*mu_HtFF(m)*(theta_knot_HtFF+ (0.75*theta_tw) - ...
                (0.75*lambda_TPP_HtFF(p))))+ ( (8/lock)*((vb^2)-1)*(beta_1s_HtFF) ) ) );
            beta_knot_HtFF =(lock/vb^2)*(((1+(mu_HtFF(m)^2))*theta_knot_HtFF/8)+((1+mu_HtFF(m)^2)*theta_tw/10)+...
                ((1/6)*mu_HtFF(m)*(beta_1c_HtFF+theta_1s_HtFF))-(lambda_TPP_HtFF(p)/6));
            theta_1c_HtFF = beta_1s_HtFF +((1/(1+(0.5*mu_HtFF(m)^2)))*((4/3*mu_HtFF(m)*beta_knot_HtFF) +((8/lock)*((vb^2)-1)*beta_1c_HtFF)));
            CH_TPP_HtFF = ((sigma*a/2)*((((1/2)*mu_HtFF(m)*lambda_TPP_HtFF(p)*(theta_knot_HtFF+(0.5*theta_tw)))) -(((1/6)*beta_knot_HtFF*theta_1c_HtFF) + ...
                ((1/4)*theta_1s_HtFF*lambda_TPP_HtFF(p)) +((1/4)*mu_HtFF(m)*beta_knot_HtFF^2)))) + (sigma*cd0*mu_HtFF(m)/4);
            CY_TPP_HtFF = -(sigma*a/2)*(((3/4)*mu_HtFF(m)*beta_knot_HtFF*(theta_knot_HtFF+((2/3)*theta_tw))) + ...
                ((1/4)*theta_1c_HtFF*lambda_TPP_HtFF(p)) +((1/6)*beta_knot_HtFF*theta_1s_HtFF*(1+(3*mu_HtFF(m)^2) )) - ...
                ((3/2)*mu_HtFF(m)*beta_knot_HtFF*lambda_TPP_HtFF(p)));
        end
